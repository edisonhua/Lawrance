...
#
# Usage:	purge.pl [ -f ] [ -d ] [ -c config.file ]
#
#		where "-f" forces a purge, regardless of when
#		the last purge occurrered, and regardless of the
#		amount of available disk space.
#
#		"-d" will set debugging on.
#
#		"-c config.file" specifies location of configuration file
#		that holds user specific configuration values to override
#		the builtin defaults.
#
# "purge.pl" (purge periodic) is run hourly by cron (as root) to purge
# /data/raw disk space.  At least one purge is run daily.  If space is
...
######################################################################################################
#
# Configuration variables that user can override via configuration file.
# If no configuration file is specified, built in defaults will be used.
#

#
##  sjames not yet  062912
##Previously it was based on %free disk space

$Free_Space_Limit          = 5;     ### Force purge if below % free
$Days_to_Purge             = 60;    ### Days beyond which to purge
$Max_time_since_last_purge = 4.0;   ### Max time between forced purges, in DAYS
$Purge_File_System_name    = "raw"; ### Target file system name (for logging)
$Purge_File_System   = "/data/raw"; ### Target file system to purge
$Base_Log_Directory = "/data/raw/log_files/Disk_Purge"; ### Log directory top
$Inp_Files_Directory = ".";         ### Directory holding required *.inp files
$Purge_Empty_Directories = 0;       ### 1=Yes, 0=Just tell about possibilities

#
# Process the argument configuration file, if user has specified one. No error
# checking is done as the script expects user to specify a valid configuration
# file as the argument.
#

my %config = do "$opt_c" if (defined($opt_c));
...
$Purge_Log_Directory_Top = $Base_Log_Directory . "/" . $Purge_File_System_name; ### Directory logs are created in.
#
...
$Purged_EmptyDirs_Log   = $Purge_Log_Directory . "/" . $local_time . "__" . $Purge_File_System_name . ".emptydirs.purged_" . $Days_to_Purge . "_Day_Purge";
#
...
#
# This subroutine expects argument file to be in $Inp_Files_Directory.
#
sub read_inp_file_into_array {
	my $file_path = $Inp_Files_Directory . "/" . $_[0];
...
@groups_to_avoid = read_inp_file_into_array("groups_to_avoid.inp");
@users_to_avoid = read_inp_file_into_array("users_to_avoid.inp");

#
### pattern ^$Purge_File_System/<paths_to_avoid>
#

@paths_to_avoid = read_inp_file_into_array("paths_to_avoid.inp");

#
### Files that need to be skipped for all the users.
#
@files_to_avoid = read_inp_file_into_array("files_to_avoid.inp");

@dirs_to_avoid = read_inp_file_into_array("dirs_to_avoid.inp");
...
#
printf(QSTAT "\nAdditional files to avoid:\n");
foreach $key ( sort(@files_to_avoid) ) {
    printf(QSTAT "%s\n",$key);
} #end_foreach $key
#
printf(QSTAT "\n");
...
#
#   Filter off files to be avoided.
#
    $avoid_file = 0;
    foreach $file_to_avoid (@files_to_avoid) {
        if ($filename =~ m/${file_to_avoid}$/) {
            $avoid_file = 1;
            last;
        } #end_if
    } #end_foreach $file_to_avoid
    next if $avoid_file;
#
...
######################################################################################################
#
# Purge Empty directories (including those that become empty due to removal of
# empty subdirectories).
#

#
# Returns 1 if arguent directory is empty, 0 otherwise.
#
sub is_empty_dir {
    my ($dir) = @_;
    my $file;
    if (opendir my $dfh, $dir) {
        ...
        # If we can not open a directory, it is as good as a non-empty directory
        # because we can not verify whether it is empty or non-empty.
        # This decision can be changed if required in future.
        return 0;
    }
}

# printf(LOG "Purging (time based) empty directories now\n\n");
# $find_opt = $Purge_File_System . " -depth -type d -atime +" . $Days_to_Purge . " -ctime +" . $Days_to_Purge . " -mtime +" . $Days_to_Purge ." ";

printf(LOG "\n\nPurging empty directories now\n\n");
$find_opt = $Purge_File_System . " -depth -type d ";

open(EDPURGED,"> $Purged_EmptyDirs_Log") or
    die("###ERROR Could not open file for $Purged_EmptyDirs_Log.");

printf(LOG "%s %s\n",$find,$find_opt);
printf(EDPURGED "%s %s\n\n",$find,$find_opt);

open(FIND,"$find $find_opt |") or
    die("###ERROR Could not open pipe from $find $find_opt");

printf(EDPURGED "Empty directories purged:\n");

$emptydirs_purged = 0;

while (defined($dirname = <FIND>)) {
    chomp($dirname);

    #
    # Filter off directories to be avoided.
    #
    $avoid_directory = 0;
    foreach $dir_to_avoid (@dirs_to_avoid) {
        chomp $dir_to_avoid;

        if ($dirname =~ m/\/${dir_to_avoid}$/) {
            $avoid_directory = 1;
            last;
        } #end_if
    } #end_foreach $dir_to_avoid
    next if $avoid_directory;

    #
    # Check if directory is empty, and delete it if allowed by configuration.
    #
    # Note that actually deleted directories could be more that shown "Will be
    # deleted", because during actual course of deletion some directories may
    # become empty if all they contained was empty subdirectories.
    #
    if (is_empty_dir($dirname)) {
        printf(EDPURGED "Dirname: %s ... ",$dirname);
        if ($Purge_Empty_Directories == 1) {
            if (rmdir($dirname)) {
                printf(EDPURGED "DELETED\n");
                $emptydirs_purged++;
            }
            else {
                printf(EDPURGED "ERROR deleting\n");
            }
        }
        else {
            printf(EDPURGED "Will be deleted\n");
        } #end_if
    } #end_if
} #end_while

printf(LOG "\n");
printf(LOG "# of empty directories purged  = %18s\n\n",$emptydirs_purged);
